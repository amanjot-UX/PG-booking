...
// FIX: was 'checkout_date' — correct column name in schema is 'checkout'
function saveBooking($data) {
    ...
    $stmt = $db->prepare("
        INSERT INTO bookings
            (user_id, property_id, checkin_date, checkout, name, email, phone, message, status, created_at)
        VALUES
            (?, ?, ?, ?, ?, ?, ?, ?, 'pending', NOW())
    ");

    if (!$stmt) {
        error_log('saveBooking prepare failed: ' . $db->error);
        return false;
    }
    ...
    $stmt->bind_param('iissssss', $userId, $propertyId, $checkin, $checkout, $name, $email, $phone, $message);

    if ($stmt->execute()) return $stmt->insert_id;

    error_log('saveBooking execute failed: ' . $stmt->error);
    return false;
}

function amenityIcon($a) {
    $icons = [
        'WiFi'=>'📶','AC'=>'❄️','Meals'=>'🍽️','Laundry'=>'👕','CCTV'=>'📹',
        'Parking'=>'🅿️','Gym'=>'💪','Power Backup'=>'⚡','Housekeeping'=>'🧹',
        'TV'=>'📺','Refrigerator'=>'🧊','Geyser'=>'🚿','Water Purifier'=>'💧',
